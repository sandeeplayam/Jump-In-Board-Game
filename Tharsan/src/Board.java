...
public class Board {
	...
	public Board() {
		...
		board = new PiecesInterface[5][5];
		Holes = new ArrayList<Hole>();

		for (int i = 0; i < 5; i++) {
			for (int j = 0; j < 5; j++) {
				board[i][j] = PieceEnums.SLOT;
	 ...
	 */
	public PiecesInterface[][] getBoard() {
		return board;
	}
	 ...
	 */
	public boolean move(PiecesInterface s, String direction) {
		// if slot is a rabbit
		if (s instanceof Rabbit) {
			...
			case "up":
				...
				//else change that position to an empty slot
					...
					board[s.getX()][s.getY()] = PieceEnums.SLOT;
					board[s.getX()][s.getY()].setPos(s.getX(), s.getY());
				}
