...
public class ActionStorageTest {
	...
	public void testAddMove() {
		/*
		 * addMove method adds the current (x, y) and new (x, y) coordinates of a game
		 * piece to two stacks one for the x and one for the y coordinate
		 */
		storage.addMove(1, 2, 3, 4, 1);

		assertEquals(3, storage.getX(1)); // here is the check to see if the correct x coordinate was pushed into the
											// movex stack
		...
		assertEquals(1, storage.getNumMoves()); /*
												 * addMove method increments currentMove by 2, so checking if it was
	...
	public void testGetY() {
		...
		storage.setMoveY(2);
		; // pushed value of 2 into the the 0th index of the movey stack
		storage.setMoveY(4);
		; // pushed value of 4 into the the 1st index of the movey stack
	...
	public void testGetundoX() {
		...
		assertEquals(1, storage.getundoX(0)); // getundoX method should return the value at the 0th indec of the redox
												// stack
	...
	public void testClearUndos() {
		// clearUndos method clears the redox and redoy stack, which means its clearing
		// the stack with all the undo moves in them
		storage.setRedoX(1);
		storage.setRedoX(3);
		storage.setRedoY(2);
		storage.setRedoY(4);
